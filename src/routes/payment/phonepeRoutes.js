...
// PUT THIS IN YOUR ROUTER FILE (replace the old /initiate-payment handler)
router.post("/initiate-payment", async (req, res) => {
  try {
    const {
      ...
      paymentPercentage: rawPaymentPercentage,
      deliveryCharges,
      couponDiscount,
      address,
      customerName,
      ...
      source,
      slotExtraCharge,
      paymentAmount, // optional from frontend
    } = req.body;
    ...
    // Normalize paymentType and paymentPercentage
    const paymentTypeNormalized =
      String(rawPaymentType || "").toLowerCase() === "full" ||
      String(rawPaymentType || "").toLowerCase() === "fullpayment"
        ? "Full"
        : String(rawPaymentType || "").toLowerCase() === "partial"
        ? "Partial"
        : null;

    const paymentPercentageNum = Number(rawPaymentPercentage);
    if (!paymentTypeNormalized || ![50, 100].includes(paymentPercentageNum)) {
      return res.status(400).json({
        message:
          "Invalid paymentType or paymentPercentage. Use Full/Partial and 50 or 100.",
        provided: { rawPaymentType, rawPaymentPercentage },
      });
    }

    // Ensure items have customizedInputs array
    ...
    // Build order document (saved as created BEFORE redirection)
    const order = new Order({
      ...
      // store the normalized fields so DB and code are consistent
    ...
    // Decide amount to send to PhonePe (in paise; integer)
    // Prefer explicit paymentAmount from frontend; otherwise calculate
    let amountToCollect =
    ...
    if (isNaN(amountToCollect) || amountToCollect <= 0) {
      ...
      await Order.findOneAndUpdate(
        { orderId },
        { paymentStatus: "FAILED", reason: "Invalid payment amount" }
      );
      return res
        .status(400)
    ...
    // PhonePe expects amount in paise (integer). Multiply by 100 and round.
    const phonePeAmountPaise = Math.round(amountToCollect * 100);

    // Get access token (your getAccessToken function)
    const accessToken = await getAccessToken();
    if (!accessToken) {
      // mark order failed
      await Order.findOneAndUpdate(
        { orderId },
        { paymentStatus: "FAILED", reason: "Failed to obtain access token" }
      );
      return res
        .status(500)
    ...
    // Build payment payload for PhonePe - adapt this to PhonePe docs
    const paymentData = {
      merchantOrderId: orderId,
      amount: phonePeAmountPaise,
      expireAfter: 1200,
      metaInfo: {
        paymentType: paymentTypeNormalized,
        paymentPercentage: paymentPercentageNum,
        isPartial: paymentTypeNormalized === "Partial",
      },
      paymentFlow: {
        type: "PG_CHECKOUT",
        message: `Payment for order ${orderId} - ${paymentPercentageNum}% ${paymentTypeNormalized}`,
        merchantUrls: {
    ...
    try {
      ...
      // expect phonePeResponse.redirectUrl (adjust per PhonePe spec)
      const paymentUrl =
        phonePeResponse?.redirectUrl || phonePeResponse?.data?.redirectUrl;

      if (!paymentUrl) {
        // treat as failure from PhonePe
        await Order.findOneAndUpdate(
          { orderId },
          {
            paymentStatus: "FAILED",
            reason: "No redirect/payment URL from PhonePe",
        ...
        return res.status(502).json({
          success: false,
          error:
            "Payment initiation failed: No payment URL from payment provider",
          providerResponse: phonePeResponse,
        });
      }

      // Respond to frontend with redirect URL
      ...
      console.error("PHONEPE ERROR FULL LOG = ", {
        status: phonePeErr.response?.status,
        headers: phonePeErr.response?.headers,
        data: phonePeErr.response?.data,
      });

      return res.status(500).json({
        success: false,
        error: "PhonePe failed",
        phonepeStatus: phonePeErr.response?.status,
        phonepeResponse: phonePeErr.response?.data,
        headers: phonePeErr.response?.headers,
      });
    }
  } catch (err) {
    console.error("Initiate-payment general error:", err);
    return res.status(500).json({
      success: false,
  ...
  try {
    ...
    if (response.data.state === "COMPLETED") {
      ...
      const payment = new Payment({
        orderId,
        customerId,
        amount: order.paidAmount, // Amount that was actually paid
        paymentType: "Online",
        status: "COMPLETED",
        paymentPercentage: order.paymentPercentage,
        isPartialPayment: order.paymentType === "Partial",
      ...
      if (populatedOrder?.customerId?.email) {
        ...
        const redirectUrl =
          order.paymentType === "Partial"
            ? `https://lavisheventzz.com/payment/partial-success?orderId=${orderId}&paidAmount=${order.paidAmount}&dueAmount=${order.dueAmount}`
