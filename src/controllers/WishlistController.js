...
export const createWishlist = async (req, res) => {
  const { customerId, serviceId, serviceName, servicePrice, serviceImages } =
    req.body;
...
export const removeItemFromWishlist = async (req, res) => {
  ...
  try {
    ...
    if (!wishlist) {
      return res
        .status(404)
        .json({ message: "Service not found in wishlist." });
    }
    ...
    res
      .status(500)
      .json({ message: "Server error while removing service from wishlist" });
  }
};
